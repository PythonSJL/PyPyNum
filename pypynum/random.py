...
def choice(seq: ite, shape: arr = None):
    if not isinstance(seq, ite):
        raise TypeError("The parameter seq must be iterable")
    if shape is None:
        return _choice(seq)
    if not (isinstance(shape, arr) and all([isinstance(item, int) and item and abs(item) == item for item in shape])):
...
def gauss(mu: real = 0, sigma: real = 1, shape: arr = None) -> float | list:
    if not (isinstance(mu, real) and isinstance(sigma, real)):
...
def randint(a: int, b: int, shape: arr = None) -> int | list:
...
def rand(shape: arr = None) -> float | list:
    if shape is None:
        return _random()
    if not (isinstance(shape, arr) and all([isinstance(item, int) and item and abs(item) == item for item in shape])):
        raise RandomError("The shape must be all positive integers")
...
def uniform(a: real, b: real, shape: arr = None) -> float | list:
