    ...
    def __matmul__(self, other):
        ...
        return Matrix(
            [[sum([self.data[i][k] * other.data[k][j] for k in range(self.cols)]) for j in range(other.cols)] for i in
             range(self.rows)], dtype=self.dtype)

    def kron(self, other):
        return Matrix(
            [[self.data[i][j] * other.data[m][n] for j in range(self.cols) for n in range(other.cols)] for i in
             range(self.rows) for m in range(other.rows)], dtype=self.dtype)

    def inner(self, other):
        if self.cols != other.cols:
            raise MatchError
        return Matrix(
            [[sum([self.data[i][k] * other.data[j][k] for k in range(self.cols)]) for j in range(other.rows)] for i in
             range(self.rows)], dtype=self.dtype)
...
def rotate90(matrix: Matrix, times: int, dtype=None) -> Matrix:
    ...
    if times % 4 == 0:
        ...
        return Matrix([list(col)[::-1] for col in zip(*matrix)], False, dtype=dtype)
    elif times % 4 == 2:
        return Matrix([row[::-1] for row in matrix[::-1]], False, dtype=dtype)
    else:
        return Matrix([list(col)[::-1] for col in zip(*matrix[::-1])][::-1], False, dtype=dtype)
...
def rank_decomp(matrix: Matrix, dtype=None) -> tuple:
    ...
    f = f[:rank, :]
    return Matrix(c, False, dtype=dtype), Matrix(f, False, dtype=dtype)
...
def lu(matrix: Matrix, dtype=None) -> tuple:
    ...
    for i in range(rows - 1):
        ...
        for j in range(i + 1, rows):
            upper[j][i] = 0.0
    return Matrix(lower, False, dtype=dtype), Matrix(upper, False, dtype=dtype)
...
def qr(matrix: Matrix, reduce: bool = False, dtype=None) -> tuple:
    ...
    if reduce:
        minimum = min(r_, c_)
        q = Matrix(q[:, :minimum], False, dtype=dtype)
        r = Matrix(r[:minimum], False, dtype=dtype)
    return q, r
