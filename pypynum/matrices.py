...
class Matrix(Array):
    """
    It is a matrix class that supports basic operations, as well as functions such as determinant and inverse matrix.
    :param data: An array in the form of a list
    :param check: Check the rationality of the input array
    :param dtype: The data type of the elements in the matrix, default is object
    """

    def __init__(self, data, check=True, dtype=object):
        super().__init__(data, check, dtype)
        if len(self.shape) != 2:
    ...
    def __matmul__(self, other):
        ...
        return Matrix(
            [[sum([self.data[i][k] * other.data[k][j] for k in range(self.cols)]) for j in range(other.cols)]
             for i in range(self.rows)], dtype=self.dtype)

    def kron(self, other):
        return Matrix(
            [[self.data[i][j] * other.data[m][n] for j in range(self.cols) for n in range(other.cols)]
             for i in range(self.rows) for m in range(other.rows)], dtype=self.dtype)

    def inner(self, other):
        if self.cols != other.cols:
            raise MatchError
        return Matrix(
            [[sum([self.data[i][k] * other.data[j][k] for k in range(self.cols)]) for j in range(other.rows)]
             for i in range(self.rows)], dtype=self.dtype)

    def outer(self, other):
        vec1, vec2 = self.flatten(), other.flatten()
        return Matrix([[a * b for b in vec2] for a in vec1], dtype=self.dtype)

    def t(self):
        return Matrix(list(map(list, zip(*self.data))), dtype=self.dtype)

    def minor(self, row, col):
        return Matrix([row[:col] + row[col + 1:] for row in self.data[:row] + self.data[row + 1:]], dtype=self.dtype)
    ...
    def det(self):
        ...
        for i in range(size):
            ...
            for k in range(i + 1, size):
                ...
                matrix[k][i + 1:] = [matrix[k][j] - factor * pivot_row[j] for j in range(i + 1, size)]
                matrix[k][i] = 0
        return det_val
    ...
    def inv(self):
        if self.rows != self.cols:
            raise SquareError
        n = self.rows
        augmented = [self.data[i] + [0 if k != i else 1 for k in range(n)] for i in range(n)]
        for i in range(n):
        ...
        return Matrix([augmented[i][n:] for i in range(n)], dtype=self.dtype)
    ...
    def rref(self, pivots=True):
        ...
        for r in range(self.rows):
            if lead >= self.cols:
                return (Matrix(matrix, dtype=self.dtype), pe) if pivots else Matrix(matrix, dtype=self.dtype)
            i = r
    ...
    def diag(self, k=0):
        return diag(self, k, dtype=self.dtype)

    def i(self):
        return identity(self.rows, self.cols, dtype=self.dtype)
    ...
    def normalize(self):
        norm = self.norm()
        return Matrix([[item / norm for item in row] for row in self.data], dtype=self.dtype)
    ...
    def basic(self, func, axis=None, dtype=None):
        ...
        if isinstance(result, list) and result and not isinstance(result[0], list):
            return Matrix([result], dtype=dtype)
        return Matrix(result, dtype=dtype)
    ...
    def latex(self, matrix_type="bmatrix", row_spacing=0, col_spacing=0, spacing_unit="mm"):
        ...
        alignment = "c" * self.cols if matrix_type == "array" else None
        ...
        latex_str = ["\\begin{{{}}}".format(matrix_type),
                     "{{{}}}".format(alignment) if alignment else "",
                     row_separator.join([col_separator.join(map(str, row)) for row in self.data]),
...
def mat(data: Any, dtype=object) -> Matrix:
    return Matrix(data, dtype=dtype)


def dctmtx(n: int, dtype=object) -> Matrix:
    ...
    for i in range(n):
        for j in range(n):
            if i == 0:
                ...
                a = (2 / n) ** 0.5
            result[i][j] = a * cos(3.141592653589793 * (j + 0.5) * i / n)
    return Matrix(result, dtype=dtype)


def rotate90(matrix: Matrix, times: int, dtype=None) -> Matrix:
    if dtype is None:
        dtype = matrix.dtype
    matrix = matrix.copy()
    if times % 4 == 0:
        ...
        return Matrix([list(col)[::-1] for col in zip(*matrix.data)], False, dtype=dtype)
    elif times % 4 == 2:
        return Matrix([row[::-1] for row in matrix.data[::-1]], False, dtype=dtype)
    else:
        return Matrix([list(col)[::-1] for col in zip(*matrix.data[::-1])][::-1], False, dtype=dtype)


def rank_decomp(matrix: Matrix, dtype=None) -> tuple:
    if dtype is None:
        dtype = matrix.dtype
    f, pivot_cols = matrix.rref()
    rank = len(pivot_cols)
    c = Matrix([[matrix[i, j] for j in pivot_cols] for i in range(matrix.rows)], False, dtype=dtype)
    f = f[:rank, :]
    return c, f


def cholesky(matrix: Matrix, hermitian: bool = True, dtype=None) -> Matrix:
    if dtype is None:
        dtype = matrix.dtype
    rows = matrix.rows
    cols = matrix.cols
    ...
    if hermitian:
        for i in range(matrix.rows):
            for j in range(i):
                lower[i][j] = (matrix[i, j] - sum([lower[i][k] * lower[j][k].conjugate() for k in range(j)])) / \
                              lower[j][j]
    ...
    return Matrix(lower, False, dtype=dtype)


def lu(matrix: Matrix, dtype=None) -> tuple:
    if dtype is None:
        dtype = matrix.dtype
    rows = matrix.rows
    cols = matrix.cols
    ...
    for i in range(rows - 1):
        ...
        for j in range(i + 1, rows):
            upper[j, i] = 0.0
    return Matrix(lower, False, dtype=dtype), upper
...
def diag(v: Any, k: int = 0, n: int = None, m: int = None, dtype=None) -> Any:
    if isinstance(v, Matrix):
        ...
        for row, col, val in zip(rows, cols, v):
            result[row][col] = val
        return Matrix(result, False, dtype=dtype)


def identity(n: int, m: int = None, dtype=object) -> Matrix:
    if m is None:
        m = n
...
def perm_mat_indices(num_rows: int, num_cols: int, row_swaps: arr = (), col_swaps: arr = ()) -> tuple:
    ...
    row_lens = set(map(len, row_swaps))
    col_lens = set(map(len, col_swaps))
    if row_lens and row_lens != {2} or col_lens and col_lens != {2}:
...
def perm_mat(num_rows: int, num_cols: int, row_swaps: arr = (), col_swaps: arr = (), rtype: Callable = Matrix,
             dtype=object) -> Any:
    ...
    if rtype is list:
        return result
    elif rtype is Matrix:
        return Matrix(result, dtype=dtype)
    else:
        return rtype(result)


def qr(matrix: Matrix, reduce: bool = False, dtype=None) -> tuple:
    if dtype is None:
        dtype = matrix.dtype
    from math import hypot
    r_ = matrix.rows
    c_ = matrix.cols
    q = identity(r_, dtype=dtype)
    r = matrix
...
def hessenberg(matrix: Matrix, dtype=None) -> tuple:
    if dtype is None:
        dtype = matrix.dtype
    a = matrix.copy()
    n = a.rows
...
def eigen(matrix: Matrix, dtype=None) -> tuple:
    if dtype is None:
        dtype = matrix.dtype
    from math import hypot
    a = matrix.copy()
    n = a.rows
    if n == 1:
        return Matrix([[float(a[0, 0])]], False, dtype=dtype), Matrix([[1.0]], False, dtype=dtype)
    q, a = hessenberg(a, dtype=dtype)
    norm = 0
    for x in range(n):
    ...
    for i in range(n):
        e[i][i] = a[i, i]
    e = Matrix(e, False, dtype=dtype)
    er = identity(n, dtype=dtype)
    m = 1
    for i in range(1, n):
        s = a[i, i]
...
def svd(matrix: Matrix, full: bool = False, calc_uv: bool = True, dtype=None) -> tuple:
    if dtype is None:
        dtype = matrix.dtype
    from math import hypot
    def svd_helper():
    ...
    if not calc_uv:
        svd_helper()
        sigma = sigma[:min(m, n)]
        return diag(sigma, dtype=dtype),
    elif full and n < m:
        v = Matrix([[0] * m for _ in range(m)], False, dtype=dtype)
        d = m - n
        u = Matrix([_ + [0] * d for _ in u.data], False, dtype=dtype)
        _, n = n, m
        svd_helper()
        sigma = sigma[:_]
        v = v[:_, :]
        return u, diag(sigma, dtype=dtype), Matrix(v, False, dtype=dtype)
    else:
        v = Matrix([[0] * n for _ in range(n)], False, dtype=dtype)
        svd_helper()
        if n > m:
            if not full:
                v = Matrix(v[:m, :], False, dtype=dtype)
            sigma = sigma[:m]
            u = Matrix(u[:, :m], False, dtype=dtype)
        return u, diag(sigma, dtype=dtype), v
