...
class IntervalSet:
    ...
    def add_interval(self, start, end, start_open=False, end_open=False):
        if start == float("-inf") or start == float("inf"): start_open = True
        if end == float("-inf") or end == float("inf"): end_open = True
        start_open, end_open = bool(start_open), bool(end_open)
        ...
        for s, e, so, eo in self.intervals:
            if s <= start < e or s < end <= e or start <= s < end or start < e <= end or (
                    start == e and not (start_open and eo)) or (s == end and not (so and end_open)):
                if start < s:
                    new_start_open = start_open
                elif start > s:
                    new_start_open = so
                else:
                    new_start_open = start_open and so
                if end > e:
                    new_end_open = end_open
                elif end < e:
                    new_end_open = eo
                else:
                    new_end_open = end_open and eo
                start = min(s, start)
                end = max(e, end)
    ...
    def remove_interval(self, start, end, start_open=False, end_open=False):
        ...
        start_open, end_open = bool(start_open), bool(end_open)
        for s, e, so, eo in self.intervals:
            if e < start or (e == start and (eo or start_open)):
                intervals.append((s, e, so, eo))
            elif s > end or (s == end and (so or end_open)):
                intervals.append((s, e, so, eo))
            else:
                if s < start or (s == start and not so and start_open):
                    intervals.append((s, start, so, not start_open))
                if e > end or (e == end and not eo and end_open):
                    intervals.append((end, e, not end_open, eo))
        self.intervals = intervals

    def is_superset(self, other):
        def contains_interval(sub_interval, super_interval):
            s1, e1, so1, eo1 = sub_interval
            s2, e2, so2, eo2 = super_interval
            left_ok = s2 <= s1 if not so2 or so1 else s2 < s1
            right_ok = e1 <= e2 if not eo2 or eo1 else e1 < e2
            return left_ok and right_ok

        for interval in other.intervals:
            if not any([contains_interval(interval, self_interval) for self_interval in self.intervals]):
                return False
        return True
    ...
    def intersection(self, other):
        ...
        while i < len(self.intervals) and j < len(other.intervals):
            ...
            if start < end or (start == end and not start_open and not end_open):
    ...
    def relative_complement(self, other):
        if not isinstance(other, IntervalSet):
            raise TypeError("Operand must be an IntervalSet")
    ...
    __or__ = union
    __sub__ = difference
    __matmul__ = relative_complement
