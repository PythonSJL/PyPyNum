...
def unary(function, right: real = 5, left: real = -5, top: real = 5, bottom: real = -5, complexity: real = 5,
    ...
    for i, d in enumerate(y):
        d = round((top - bottom - d - d) * complexity / 2 / ratio)
        if 0 <= d <= len(y) - 1:
...
def c_unary(function, start: real, end: real, interval: real = 5, projection: str = "ri", right: real = 5,
    ...
    if projection == "ri":
        ...
        x = linspace(left, right, round((right - left) * interval + 1))
    ...
    for p, _ in enumerate(c):
        ...
        c0, c1 = round((right - left + _c0 + _c0) * complexity / 2), round(
            (top - bottom - _c1 - _c1) * complexity / 2 / ratio)
