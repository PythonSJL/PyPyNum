...
def caesar(text: str, shift: int, decrypt: bool = False) -> str:
    ...
    for char in text:
        if char.isalpha():
            ascii_offset = __ORD_a if char.islower() else __ORD_A
...
def vigenere(text: str, key: str, decrypt: bool = False) -> str:
    ...
    for char in text:
        if char.isalpha():
            ...
            shift = ord(key_char.lower()) - __ORD_a
            if decrypt:
                shift = 26 - shift
            if char.isupper():
                result += chr((ord(char) - __ORD_A + shift) % 26 + __ORD_A)
            else:
                result += chr((ord(char) - __ORD_a + shift) % 26 + __ORD_a)
        else:
            result += char
...
def morse(text: str, decrypt: bool = False) -> str:
    code_dict = __MORSE_CODE_REVERSE if decrypt else __MORSE_CODE
    result = []
    if decrypt:
        text = text.split()
    else:
...
def hill256(text: bytes, key: list, decrypt: bool = False) -> bytes:
    ...
    if decrypt:
        key = decrypt_key(key)
    cols = key.cols
    result = key @ mat(fill([cols, length // cols + bool(length % cols)], tuple(text), False)) % mod
