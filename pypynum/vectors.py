...
class Vector(Array):
    """
    It is a vector class that supports basic operations, as well as functions such as norm and all angles.
    :param data: An array in the form of a list
    :param check: Check the rationality of the input array
    :param dtype: The data type of the elements in the vector, default is object
    """
    ...
    def __pos__(self):
        return Vector([x for x in self.data], dtype=self.dtype)
    ...
    def normalize(self):
        norm = self.norm()
        return Vector([x / norm for x in self.data], dtype=self.dtype)
