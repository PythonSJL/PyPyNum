...
class Quaternion:
    ...
    def exp(self):
        ...
        w, x, y, z = self.__quaternion
        v_norm = (x ** 2 + y ** 2 + z ** 2) ** 0.5
        s_exp = exp(w)
        if v_norm == 0:
            return Quaternion(s_exp, 0, 0, 0)
        coeff = s_exp * sin(v_norm) / v_norm
        return Quaternion(s_exp * cos(v_norm), x * coeff, y * coeff, z * coeff)
    ...
    def log(self):
        ...
        q_norm = self.norm()
        if q_norm == 0:
            raise ValueError("Logarithm of zero quaternion is undefined")
        scalar_part = log(q_norm)
        v_norm = (x ** 2 + y ** 2 + z ** 2) ** 0.5
        if v_norm == 0:
        ...
        angle = acos(ratio)
        coeff = angle / v_norm
        return Quaternion(scalar_part, x * coeff, y * coeff, z * coeff)
...
class Octonion:
    ...
    def exp(self):
        from math import exp, cos, sin
        s, t, u, v, w, x, y, z = self.__octonion
        v_norm = (t ** 2 + u ** 2 + v ** 2 + w ** 2 + x ** 2 + y ** 2 + z ** 2) ** 0.5
        s_exp = exp(s)
        if v_norm == 0:
            return Octonion(s_exp, 0, 0, 0, 0, 0, 0, 0)
        coeff = s_exp * sin(v_norm) / v_norm
        return Octonion(s_exp * cos(v_norm), t * coeff, u * coeff, v * coeff,
                        w * coeff, x * coeff, y * coeff, z * coeff)
    ...
    def log(self):
        ...
        o_norm = self.norm()
        if o_norm == 0:
            raise ValueError("Logarithm of zero octonion is undefined")
        scalar_part = log(o_norm)
        v_norm = (t ** 2 + u ** 2 + v ** 2 + w ** 2 + x ** 2 + y ** 2 + z ** 2) ** 0.5
        if v_norm == 0:
            return Octonion(scalar_part, 0, 0, 0, 0, 0, 0, 0)
        ratio = s / o_norm
        if ratio > 1:
            ratio = 1
        if ratio < -1:
        ...
        angle = acos(ratio)
        coeff = angle / v_norm
        return Octonion(scalar_part, t * coeff, u * coeff, v * coeff, w * coeff, x * coeff, y * coeff, z * coeff)
