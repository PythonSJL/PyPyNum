...
def geometric_pmf(k, p=0.5):
    ...
    Returns the probability of achieving the first success on the k-th trial.
    Definition Note:
    This function uses the definition where k is the number of trials needed 
    to get the first success (support k = 1, 2, 3, ...).
    P(X=k) = p * (1-p)^(k-1)
...
def hypergeom_pmf(M, n, N, k):
    ...
    Example
    ==========
    >>> hypergeom_pmf(5, 3, 2, 2)
    0.3

    :param M: The total number of items in the population, must be a non-negative integer.
    :param n: The number of successes in the population, must be a non-negative integer less than or equal to M.
    :param N: The number of items in the sample, must be a non-negative integer less than or equal to M.
    :param k: The number of successes in the sample, must be a non-negative integer.
    :return: The probability of drawing k successes in a sample of size N from a population with M items and n successes.
    """
    try:
        from math import comb
    except ImportError:
        from .maths import combination as comb
    if not (isinstance(M, int) and M >= 0) or not (isinstance(n, int) and n >= 0) or not (
...
def nbinom_pmf(k, n=1, p=0.5):
    ...
    :param n: The number of failures until the experiment is stopped, must be a positive integer.
    :param p: The probability of success on each trial, must be between 0 and 1.
    :return: The probability of getting k successes before n failures occur in a sequence of trials with probability p of success on each trial.
    """
    if not 0 <= p <= 1:
        raise ValueError("Probability p must be between 0 and 1")
    if not k >= 0:
    ...
    if not isinstance(n, int) or not isinstance(k, int):
        raise ValueError("n and k must be integers")
    return _factorial(n + k - 1) / (_factorial(k) * _factorial(n - 1)) * (p ** k) * ((1 - p) ** n)


def nhypergeom_pmf(M, n, r, k):
    """
    Introduction
    ==========
    ...
    Example
    ==========
    >>> nhypergeom_pmf(10, 4, 2, 2)
    0.21428571428571427

    :param M: The total number of items in the population, must be a non-negative integer.
    :param n: The number of successes in the population, must be a non-negative integer less than or equal to M.
    :param r: The number of successes to be obtained, must be a non-negative integer less than or equal to n.
    :param k: The number of failures before getting r successes, must be a non-negative integer.
    :return: The probability of observing k failures before observing r successes when drawing without replacement from a population with M items and n successes.
    """
    try:
        from math import comb
    except ImportError:
    ...
    if k + r > M:
        raise ValueError("k + r must be less than or equal to M")
    return comb(r + k - 1, r - 1) * comb(M - r - k, n - r) / comb(M, n)
...
def pareto_pdf(x, b, scale):
    ...
    :param x: The value at which to evaluate the PDF. It must be greater than or equal to the scale parameter.
    :param b: The shape parameter of the distribution, must be positive.
    :param scale: The scale parameter of the distribution, must be positive.
    :return: The probability density at x for the Pareto distribution with shape parameter b and scale parameter scale.
    """
    if b <= 0 or scale <= 0:
        raise ValueError("Both b and scale parameters must be positive")
    if x < scale:
        raise ValueError("x must be greater than or equal to the scale parameter")
    return b * scale ** b / x ** (b + 1)
...
def uniform_pdf(x, loc=0, scale=1):
    ...
    :param scale: The scale parameter, the width of the interval over which the distribution is defined, must be positive.
