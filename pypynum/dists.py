from math import (atan as _atan, cos as _cos, erf as _erf, exp as _exp, factorial as _factorial, gamma as _gamma,
                  log as _log, sqrt as _sqrt)


def beta_pdf(x, alpha=1.0, beta=1.0):
    """
    Introduction
    ==========
    ...
    :param alpha: The first shape parameter, must be positive.
    :param beta: The second shape parameter, must be positive.
    :return: The probability density at x for the Beta distribution with parameters alpha and beta.
    """
    if alpha <= 0 or beta <= 0:
        raise ValueError("Shape parameters alpha and beta must be positive")
    if x < 0 or x > 1:
        return 0
    return (x ** (alpha - 1) * (1 - x) ** (beta - 1)) / _gamma(alpha) / _gamma(beta) * _gamma(alpha + beta)
...
def cauchy_pdf(x, x0, gamma):
    ...
    :return: The probability density at x for the Cauchy distribution with location parameter x0 and scale parameter gamma.
    """
    if gamma <= 0:
...
def cauchy_cdf(x, x0, gamma):
    ...
    :param x0: The location parameter of the distribution.
    :param gamma: The scale parameter of the distribution, must be positive.
    :return: The cumulative probability at x for the Cauchy distribution with location parameter x0 and scale parameter gamma.
    """
    if gamma <= 0:
        raise ValueError("Scale parameter gamma must be positive")
...
def expon_pdf(x, lam=1.0):
    ...
    :param x: The value at which to evaluate the PDF, must be non-negative.
    :param lam: The rate parameter (lambda), must be positive.
    :return: The probability density at x for the Exponential distribution with the given rate parameter lam.
    """
    if lam <= 0:
        raise ValueError("Rate parameter must be positive")
    return lam * _exp(-lam * x) if x >= 0 else 0


def expon_cdf(x, lam=1.0):
    """
    Introduction
    ==========
    ...
    :param lam: The rate parameter (lambda), must be positive.
    :return: The cumulative probability at x for the Exponential distribution with the given rate parameter lam.
    """
    if lam <= 0:
        raise ValueError("Rate parameter must be positive")
    if x < 0:
...
def geometric_pmf(k, p=0.5):
    ...
    Introduction
    ==========
    Geometric probability mass function. Returns the probability of achieving the first success on the k-th trial.

    Definition Note: This function uses the definition where k is the number of trials needed to get the first success (support k = 1, 2, 3, ...).
    P(X=k) = p * (1-p)^(k-1)
    ...
    :param k: The number of trials until the first success, must be a positive integer (k >= 1).
    ...
    if not 0 <= p <= 1:
        raise ValueError("Probability p must be between 0 and 1")
    if k < 1:
        raise ValueError("k must be a positive integer (k >= 1) for this definition")
...
def hypergeom_pmf(k, N, K, n):
    ...
    :param k: The number of successes in the sample, must be a non-negative integer.
    :param N: The total number of items in the population, must be a non-negative integer.
    :param K: The number of successes in the population, must be a non-negative integer less than or equal to N.
    :param n: The number of items in the sample, must be a non-negative integer less than or equal to N.
    :return: The probability of drawing k successes in a sample of size n from a population with N items and K successes.
    """
    try:
        from math import comb
    except ImportError:
        from .maths import combination as comb
    if not (isinstance(N, int) and N >= 0) or not (isinstance(K, int) and K >= 0) or not (
            isinstance(n, int) and n >= 0) or not (isinstance(k, int) and k >= 0):
...
def invgauss_pdf(x, mu, lam):
    ...
    :param lam: The shape parameter of the distribution, must be positive.
    :return: The probability density at x for the inverse Gaussian distribution with mean mu and shape parameter lam.
    """
    if mu <= 0 or lam <= 0:
        raise ValueError("mu and lam parameters must be positive")
    if x <= 0:
        raise ValueError("x must be positive")
    return _sqrt(lam / (6.283185307179586 * x ** 3)) * _exp(-lam * (x - mu) ** 2 / (2 * mu ** 2 * x))
...
def log_logistic_pdf(x, alpha, beta):
    ...
    :return: The probability density at x for the log-logistic distribution with scale parameter alpha and shape parameter beta.
    """
    if alpha <= 0 or beta <= 0:
...
def log_logistic_cdf(x, alpha, beta):
    ...
    :param alpha: The scale parameter of the distribution, must be positive.
    :param beta: The shape parameter of the distribution, must be positive.
    :return: The cumulative probability at x for the log-logistic distribution with scale parameter alpha and shape parameter beta.
    """
    if alpha <= 0 or beta <= 0:
        raise ValueError("alpha and beta parameters must be positive")
    if x <= 0:
...
def logistic_pdf(x, loc=0, scale=1):
    """
    Introduction
    ==========
    ...
    :param scale: The scale parameter of the distribution, must be positive.
    :return: The probability density at x for the logistic distribution with location parameter loc and scale parameter scale.
    """
    if scale <= 0:
        raise ValueError("Scale parameter scale must be positive")
    return _exp(-(x - loc) / scale) / (scale * (1 + _exp(-(x - loc) / scale)) ** 2)
...
def logistic_cdf(x, loc=0, scale=1):
    ...
    :param loc: The location parameter of the distribution.
    :param scale: The scale parameter of the distribution, must be positive.
    :return: The cumulative probability at x for the logistic distribution with location parameter loc and scale parameter scale.
    """
    if scale <= 0:
        raise ValueError("Scale parameter scale must be positive")
    return 1 / (1 + _exp(-(x - loc) / scale))
...
def lognorm_pdf(x, mu=0, sigma=1):
    ...
    :param x: The value at which to evaluate the PDF, must be non-negative.
    :param mu: The mean of the logarithm of the distribution (location parameter).
    :param sigma: The standard deviation of the logarithm of the distribution (scale parameter), must be positive.
    :return: The probability density at x for the Lognormal distribution with the given mu and sigma.
    """
    if sigma <= 0:
        raise ValueError("Sigma parameter must be positive")
    if x <= 0:
        return 0
    return 1 / (x * sigma * 2.5066282746310002) * _exp(-((_log(x) - mu) ** 2) / (2 * sigma ** 2))
...
def logser_pmf(k, p):
    ...
    :param k: The number of occurrences, must be a positive integer.
    :param p: The parameter of the distribution, must be a positive number less than 1.
    :return: The probability of observing k occurrences in a logarithmic distribution with parameter p.
    """
...
def multinomial_pmf(k, n, p):
    ...
    :param k: A list of integers representing the number of successes in each category. The length of k must match the length of p.
...
def nbinom_pmf(k, r=1, p=0.5):
    ...
    :param r: The number of failures until the experiment is stopped, must be a positive integer.
    :param p: The probability of success on each trial, must be between 0 and 1.
    :return: The probability of getting k successes before r failures occur in a sequence of trials with probability p of success on each trial.
    """
    if not 0 <= p <= 1:
        raise ValueError("Probability p must be between 0 and 1")
    if not k >= 0:
    ...
    if not isinstance(r, int) or not isinstance(k, int):
        raise ValueError("r and k must be integers")
    return _factorial(r + k - 1) / (_factorial(k) * _factorial(r - 1)) * (p ** k) * ((1 - p) ** r)


def nhypergeom_pmf(k, N, K, r):
    """
    Introduction
    ==========
    ...
    :param N: The total number of items in the population, must be a non-negative integer.
    :param K: The number of successes in the population, must be a non-negative integer less than or equal to N.
    :param r: The number of successes to be obtained, must be a non-negative integer less than or equal to K.
    :return: The probability of observing k failures before observing r successes when drawing without replacement from a population with N items and K successes.
    """
    try:
        from math import comb
    except ImportError:
        from .maths import combination as comb
    if not (isinstance(N, int) and N >= 0) or not (isinstance(K, int) and K >= 0) or not (
            isinstance(r, int) and r >= 0) or not (isinstance(k, int) and k >= 0):
        raise ValueError("All parameters must be non-negative integers")
    if K > N:
        raise ValueError("K must be less than or equal to N")
    if r > K:
        raise ValueError("r must be less than or equal to K")
    if k + r > N:
        raise ValueError("k + r must be less than or equal to N")
    return comb(r + k - 1, r - 1) * comb(N - r - k, K - r) / comb(N, K)
...
def pareto_pdf(x, alpha, x_m):
    ...
    :param x: The value at which to evaluate the PDF. It must be greater than or equal to x_m.
    :param alpha: The shape parameter of the distribution, must be positive.
    :param x_m: The scale parameter (minimum value/threshold) of the distribution, must be positive.
    :return: The probability density at x for the Pareto distribution with shape parameter alpha and scale parameter x_m.
    """
    if alpha <= 0 or x_m <= 0:
        raise ValueError("Both alpha and x_m parameters must be positive")
    if x < x_m:
        raise ValueError("x must be greater than or equal to x_m")
...
def poisson_pmf(k, lam=1):
    ...
    :param lam: The rate parameter (lambda) of the distribution, must be non-negative real number.
    :return: The probability of k successes occurring in a given time period or space.
    """
    if k < 0:
        raise ValueError("k must be non-negative")
    return (lam ** k * _exp(-lam)) / _factorial(k)
...
def t_pdf(x, df=1):
    ...
    if df <= 0:
        raise ValueError("Degrees of freedom must be positive")
    return _gamma((df + 1) / 2) / (_sqrt(df * 3.141592653589793) * _gamma(df / 2)) * (1 + x ** 2 / df) ** (
            -(df + 1) / 2)
...
def uniform_pdf(x, a=0, b=1):
    ...
    :param b: The upper bound of the distribution, must be greater than a.
    :return: The probability density at x for the uniform distribution between a and b.
    """
    if b <= a:
...
def uniform_cdf(x, a=0, b=1):
    ...
    :param a: The lower bound of the distribution.
    :param b: The upper bound of the distribution, must be greater than a.
    :return: The cumulative probability at x for the uniform distribution between a and b.
    """
    if b <= a:
        raise ValueError("Upper bound b must be greater than lower bound a")
...
def weibull_max_pdf(x, k=1, scale=1, loc=0):
    """
    Introduction
    ==========
    ...
    if x > loc:
        return 0
    return (k / scale) * ((loc - x) / scale) ** (k - 1) * _exp(-((loc - x) / scale) ** k)


def weibull_min_pdf(x, k=1, scale=1, loc=0):
    """
    Introduction
    ==========
    ...
    if x < loc:
        return 0
    return (k / scale) * ((x - loc) / scale) ** (k - 1) * _exp(-((x - loc) / scale) ** k)
...
def zipf_pmf(k, s, n):
    ...
    :return: The probability mass at rank k for the Zipf distribution with exponent parameter s and number of elements n.
    """
    if s <= 0:
